...
class MP4AdBufferingTimeComponentTest: XCTestCase {
    ...
    func testStartBuffering() {
        ...
        guard case .inProgress = sut else { XCTFail("on play mp4 Ad sut should be .inProgress, actual \(sut)"); return }
    }
    
    func testFinishBuffering() {
        let initialDate = Date()
        let initial = MP4AdBufferingTime.inProgress(startAt: initialDate)
        
        let sut = reduce(state: initial, action: AdPlaybackBufferingInactive())
        
        guard case let .finished(startAt, _) = sut else {
    ...
    func testResetState() {
        ...
        actions.forEach { action in
            ...
            guard case .empty = sut else { XCTFail("on \(action) sut should be .empty, actual \(sut)"); return }
        }
    }
}
